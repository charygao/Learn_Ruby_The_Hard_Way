class Game
  ...
  def play()
    ...
    next_room = @start
puts "@是取址符next_room内容是%s，类型是%s" % [next_room,next_room.class]
    while true
      puts "\n--------"
      room = method(next_room)
puts "room内容是%s，类型是%s" % [room,room.class]
      next_room = room.call()
    end
  end


  def initialize(startss)
    @quips = [
		"死法1",
		"死法2",
		"死法3",
    ...
    @start = startss
	puts "startss内容是%s，类型是%s" % [startss,startss.class]
